    ...
    public void run() {
        ...
        try {
            ...
            while ((msg = in.readLine()) != null) {
                ...
                }*/else if (msg.equals(CommunicationType.MSG_SEND.toString())){
                    String ivTemp = in.readLine();
                    String chatRoomId = in.readLine();
                    String cipherText = in.readLine();
                    String macText = in.readLine();

                    byte[] ivPerMessage = Base64.getDecoder().decode(ivTemp);
                    String decryptedMessage = EncryptionUtil.decrypt(cipherText,KEYKEYKEYKEY,ivPerMessage);

                    consumeMessage(decryptedMessage + " \n");
                }else if (msg.equals(CommunicationType.CHATROOM_JOIN.toString())){
                    String chrKey = in.readLine();
                    byte[] decodedKey = Base64.getDecoder().decode(chrKey);
                    SecretKey chatroomKey = new SecretKeySpec(decodedKey,0,decodedKey.length,"AES");
                    _client.setChatRoomKey(chatroomKey);
                }
